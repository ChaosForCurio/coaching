...
import { bn as object, bo as string, bp as number, bq as array, br as union, bs as date, bt as boolean, bu as record, a as s, bv as preprocess, bw as _function, bx as promise, by as any, bz as unknown } from "./sequence_DmJMvBqm.mjs";
...
function resolveTagName$1(name, isAttribute2, options, matcher, xmlVersion) {
...
function arrToStr(arr, options, indentation, matcher, stopNodeExpressions, xmlVersion) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    const tagName = isSpecialName ? rawTagName : resolveTagName$1(rawTagName, false, options, matcher, xmlVersion);
...
function processTextOrObjNode(object2, key, level, matcher, xmlVersion) {
  ...
  const isStopNode = this.checkStopNode(matcher);
  if (isStopNode) {
    const rawContent = this.buildRawContent(object2);
...
async function GET(context) {
  ...
  return getRssResponse({
    ...
    items: sortedBlog.map((post) => ({
      ...
      description: post.data.excerpt || `Read the latest tech insights on ${post.data.title}.`,
