import { d as db, b as attendance } from './schema_CuVt3FVI.mjs';
...
import { e as eventBus } from './eventBus_CH1azXgE.mjs';
...
const POST = async ({ request, cookies }) => {
  try {
    ...
    if (toInsert.length > 0) {
      await db.insert(attendance).values(toInsert).onConflictDoNothing({ target: [attendance.student_id, attendance.course_id, attendance.date] });
    }
