...
const crypto$1 = {
  randomBytes: (l) => Crypto.getRandomValues(Buffer$1.alloc(l)),
  ...
  createHash: (type) => ({
    update: (x) => ({
      digest: (encoding) => {
        ...
        if (encoding === "hex") {
          return prom.then((arrayBuf) => Buffer$1.from(arrayBuf).toString("hex"));
        } else if (encoding) {
  ...
  async function read() {
    try {
      ...
      while ({ done, value } = await tcp.reader.read(), !done)
        tcp.emit("data", Buffer$1.from(value));
    } catch (err) {
      error(err);
    }
  }
...
const types = {
  ...
  bytea: {
    to: 17,
    from: 17,
    serialize: (x) => "\\x" + Buffer$1.from(x).toString("hex"),
    parse: (x) => Buffer$1.from(x.slice(2), "hex")
  }
};
class NotTagged {
  then() {
...
const size = 256;
let buffer = Buffer$1.allocUnsafe(size);
...
const b = Object.assign(reset, messages, {
  ...
  str(x) {
    const length = Buffer$1.byteLength(x);
    fit(length);
  ...
  raw(x) {
    buffer = Buffer$1.concat([buffer.subarray(0, b.i), x]);
    b.i = buffer.length;
    return b;
  },
  end(at = 1) {
    ...
    b.i = 0;
    buffer = Buffer$1.allocUnsafe(size);
    return out;
  }
});
function fit(x) {
  if (buffer.length - b.i < x) {
    ...
    buffer = Buffer$1.allocUnsafe(length + (length >> 1) + x);
    prev.copy(buffer);
  }
}
function reset() {
...
let uid = 1;
const Sync = b().S().end(), Flush = b().H().end(), SSLRequest = b().i32(8).i32(80877103).end(8), ExecuteUnnamed = Buffer$1.concat([b().E().str(b.N).i32(0).end(), Sync]), DescribeUnnamed = b().D().str("S").str(b.N).end(), noop$1 = () => {
};
...
function Connection(options, queues = {}, { onopen = noop$1, onend = noop$1, onclose = noop$1 } = {}) {
  ...
  let socket = null, cancelMessage, errorResponse = null, result = new Result(), incoming = Buffer$1.alloc(0), needsTypes = options.fetch_types, backendParameters = {}, statements = {}, statementId = Math.random().toString(36).slice(2), statementCount = 1, closedTime = 0, remaining = 0, hostIndex = 0, retries = 0, length = 0, delay = 0, rows = 0, serverSignature = null, nextWriteTimer = null, terminated = false, incomings = null, results = null, initial = null, ending = null, stream = null, chunk = null, ended = null, nonce = null, query = null, final = null;
  const connection2 = {
    queue: queues.closed,
    idleTimer,
    connect(query2) {
  ...
  function toBuffer(q) {
    ...
    return q.options.simple ? b().Q().str(q.statement.string + b.N).end() : q.describeFirst ? Buffer$1.concat([describe(q), Flush]) : q.prepare ? q.prepared ? prepared(q) : Buffer$1.concat([describe(q), prepared(q)]) : unnamed(q);
  }
  function describe(q) {
  ...
  function write(x, fn) {
    chunk = chunk ? Buffer$1.concat([chunk, x]) : Buffer$1.from(x);
  ...
  function data(x) {
    ...
    incoming = incomings ? Buffer$1.concat(incomings, length - remaining) : incoming.length === 0 ? x : Buffer$1.concat([incoming, x], incoming.length + x.length);
  ...
  async function AuthenticationMD5Password(x) {
    const payload = "md5" + await md5(
      Buffer$1.concat([
        Buffer$1.from(await md5(await Pass() + user)),
        x.subarray(9)
      ])
    );
    write(
  ...
  async function SASLContinue(x) {
    ...
    const payload = "c=biws,r=" + res.r + ",p=" + xor(
      clientKey,
      Buffer$1.from(await hmac(await sha256(clientKey), auth))
    ).toString("base64");
    write(
      b().p().str(payload).end()
    );
...
function xor(a, b2) {
  ...
  const buffer2 = Buffer$1.allocUnsafe(length);
...
function Subscribe(postgres2, options) {
  ...
  async function init(sql3, slot2, publications) {
    ...
    const state2 = {
      lsn: Buffer$1.concat(x.consistent_point.split("/").map((x2) => Buffer$1.from(("00000000" + x2).slice(-8), "hex")))
    };
...
function Postgres(a, b2) {
  ...
  async function listen(name, fn, onlisten) {
    ...
    channels[name] = { result: sql22`listen ${sql22.unsafe('"' + name.replace(/"/g, '""') + '"')}`, listeners: [listener] };
    ...
    async function unlisten() {
      ...
      return sql22`unlisten ${sql22.unsafe('"' + name.replace(/"/g, '""') + '"')}`;
    }
  }
  ...
  async function begin(options2, fn) {
    ...
    async function scope(c, fn2, name) {
      ...
      sql22.prepare = (x) => prepare = x.replace(/[^a-z0-9$-_. ]/gi);
      let uncaughtError, result;
      name && await sql22`savepoint ${sql22(name)}`;
      try {
        ...
        await (name ? sql22`rollback to ${sql22(name)}` : sql22`rollback`);
...
function parseOptions(a, b2) {
  if (a && a.shared)
    return a;
  const env = process$1.env, o = (!a || typeof a === "string" ? b2 : a) || {}, { url, multihost } = parseUrl(a), query = [...url.searchParams].reduce((a2, [b22, c]) => (a2[b22] = c, a2), {}), host = o.hostname || o.host || multihost || url.hostname || env.PGHOST || "localhost", port = o.port || url.port || env.PGPORT || 5432, user = o.user || o.username || url.username || env.PGUSERNAME || env.PGUSER || osUsername();
...
function requireMain() {
  ...
  function _dotenvKey(options) {
    ...
    if (define_process_env_default2.DOTENV_KEY && define_process_env_default2.DOTENV_KEY.length > 0) {
      return define_process_env_default2.DOTENV_KEY;
    }
    return "";
  }
  ...
  function _configVault(options) {
    const debug = parseBoolean(define_process_env_default2.DOTENV_CONFIG_DEBUG || options && options.debug);
    const quiet = parseBoolean(define_process_env_default2.DOTENV_CONFIG_QUIET || options && options.quiet);
    if (debug || !quiet) {
      _log("loading env from encrypted .env.vault");
    }
    const parsed = DotenvModule._parseVault(options);
    let processEnv = define_process_env_default2;
...
dotenv.config();
const connectionString = define_process_env_default.DATABASE_URL || "postgresql://postgres.qttzzkrwijtxnmeukmlx:Yq603SikoeJ3hTPV@aws-1-ap-southeast-1.pooler.supabase.com:5432/postgres";
